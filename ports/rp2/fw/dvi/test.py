...
# color.
pixel_group = [
    array.array("I", [0xFF00FF00]),
    array.array("I", [0x02020202]),
    array.array("I", [0x07070707]),
    array.array("I", [0x55005500]),
]
...
data = array.array("I")
lof7 = []
for i in range(V_TOTAL_LINES):
    if i < V_FRONT_PORCH:
        data += vblank_line_vsync_off
    elif i < (V_FRONT_PORCH + V_SYNC_WIDTH):
        data += vblank_line_vsync_on
    elif i < (V_FRONT_PORCH + V_SYNC_WIDTH + V_BACK_PORCH):
        data += vblank_line_vsync_off
    else:
        line = i - (V_FRONT_PORCH + V_SYNC_WIDTH + V_BACK_PORCH)
        data += vactive_line
        for j in range(40):
            lof7.append(len(data) + 1)
            data += pixbloc

print(len(data), machine.freq() / (5 * V_TOTAL_LINES * H_TOTAL_PIXELS))
...
# Configure HSTX's TMDS encoder for RGB565
expand_tmds = hstx.pack_expand_tmds(
    l2_nbits=4, l2_rot=0, l1_nbits=5, l1_rot=27, l0_nbits=4, l0_rot=21
)
...
# entire 32-bit word.
expand_shift = hstx.pack_expand_shift(enc_n_shifts=2, enc_shift=16, raw_n_shifts=1, raw_shift=0)
