...
#define HR_STATE_NULL   (0)
#define HR_STATE_READY  (1)
#define HR_STATE_REQ    (2)
...
typedef struct mp_obj_voice {
    mp_obj_base_t base;
    Voice_t voice;
    struct mp_obj_voice * prev;
    struct mp_obj_voice * next;
} mp_obj_voice_t;

_Bool voice_fill_bloc(Voice_t * voice, int32_t * amp, int32_t * dest);

static inline void _voice_link(mp_obj_voice_t * self, mp_obj_voice_t * head) {
    self->prev = head;
    self->next = head->next;
    if (head->next) {
        head->next->prev = self;
...
static inline  mp_obj_voice_t * _voice_unlink(mp_obj_voice_t * self) {
    mp_obj_voice_t * next = self->next;
    mp_obj_voice_t * prev = self->prev;
    if (next) {
        next->prev = prev;
    }
    if (prev) {
